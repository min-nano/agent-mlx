#!/usr/bin/env bash
#
# install-update.sh — 自動アップデートの差し替えスクリプト（macOS 専用）。
#
# 実行中の .app は自分自身を置き換えられないため、アプリ（UpdaterService）が
# このスクリプトを切り離したプロセスとして起動してから終了する。スクリプトは
# アプリの終了を待ち、ステージ済みの新しい .app で置き換えて再起動する。
#
# 原本は scripts/install-update.sh で、macOS アプリのビルド（project.yml の
# resources 指定）が .app の Contents/Resources へ同梱する。手で実行するもの
# ではない。
#
...
SRC="${2:?staged .app}"
DST="${3:?target .app}"
LOG="${TMPDIR:-/tmp}/mlxchat-update.log"
