...
#     scripts/wait-pr-checks.sh --pr 1 >/tmp/pr1.log 2>&1
#   を投げて別作業を続け、終了通知が来たら /tmp/pr1.log を Read するだけ。
#
...
# `total_count: 0` かつ `state: "pending"` を返し続ける。姉妹リポジトリ
# min-nano/photogrammetry の PR #7 では全 5 チェックが 2 分で success になったのに、
# この待ち方をしたコマンドは 27 分たっても exit しなかった（実際に踏んだ）。
#
# したがってこのスクリプトは
#
...
MLX_REPO="${MLX_REPO:-min-nano/agent-mlx}"
MLX_API="https://api.github.com/repos/${MLX_REPO}"
...
resolve_sha() {
	...
	if [ -n "$PR" ]; then
		json="$(api "$MLX_API/pulls/$PR")" || return 1
		...
		json="$(api "$MLX_API/commits/$REF")" || return 1
...
snapshot() {
	...
	runs="$(api "$MLX_API/actions/runs?head_sha=$sha&per_page=100")" || return 1
	checks="$(api "$MLX_API/commits/$sha/check-runs?per_page=100&filter=latest")" || return 1
	st="$(api "$MLX_API/commits/$sha/status?per_page=100")" || return 1
...
finish() {
	...
	echo
	echo "sha=$sha"
	echo "commit_url=https://github.com/$MLX_REPO/commit/$sha"
	[ -z "$PR" ] || echo "pr_url=https://github.com/$MLX_REPO/pull/$PR"
	if [ -n "$snap" ]; then
...
note "wait-pr-checks: repo=$MLX_REPO ${PR:+pr=$PR}${REF:+ref=$REF}${SHA:+sha=$SHA} poll=${POLL}s timeout=${TIMEOUT}s"
...
while true; do
	...
	if [ "$seen" -eq 0 ]; then
		...
		# ジョブが増える瞬間（例: build-macos 完了 → release が現れるまでの数秒）に
