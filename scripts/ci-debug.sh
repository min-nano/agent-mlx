...
#     build        swift build（Sources/ の純ロジックだけ。MLX を引かないので速い）
#     test         swift test（--args に追加フラグ。例 '--filter UpdateFeedTests'）
#     xcode-mac    xcodebuild で macOS アプリをビルド（MLX を丸ごとビルドするので重い）
#     xcode-ios    xcodebuild で iOS アプリをビルド（実機向け・未署名）
#     xcode-cli    xcodebuild で mlxchat-cli をビルド
#     run-cli      mlxchat-cli をビルドして実行（--args が CLI の引数）
...
#   CI_DEBUG_TIMEOUT          待機の上限・秒（既定 5400 = 90 分。ジョブ側の
#                             timeout-minutes と同じ。MLX を丸ごとビルドする
#                             xcode-* モードは初回 30 分以上かかる）
#
...
resolve_run() {
	...
	while [ "$tries" -lt 30 ]; do
		tries=$((tries + 1))
		id="$(api "$MLX_API/actions/workflows/$WORKFLOW_FILE/runs?event=workflow_dispatch&per_page=50" |
...
wait_run() {
	...
	started="$(date +%s)"
	while true; do
		json="$(api "$MLX_API/actions/runs/$id")"
...
fetch_payload() {
	...
	tmp="$(mktemp)"
	url="$(api -o /dev/null -w '%{redirect_url}' "$MLX_API/actions/jobs/$job/logs")"
	if [ -n "$url" ]; then
		...
		api "$MLX_API/actions/jobs/$job/logs" >"$tmp"
	fi
	rm -f "$tmp.err"
...
report() {
	...
	echo "run_url=https://github.com/$MLX_REPO/actions/runs/$id"
	echo
	fetch_payload "$id"
	[ "$conclusion" = "success" ]
}
