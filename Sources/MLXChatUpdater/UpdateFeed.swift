...
//
//  1 リリースには **2 つのアセット**が付く:
//    MLXChat.app.zip   macOS 版（自動アップデートが差し替えるのはこちら）
//    MLXChat.ipa       iOS 版（署名されていないので、アプリからは更新できない。
//                      画面にはダウンロード先として出す）
//  片方しか無いリリースもあり得る（ビルドが片方だけ通った場合）ので、
//  どちらも省略可能として扱い、両方無いリリースだけを一覧から落とす。
//
...
public struct UpdateChannel: Equatable, Sendable
{
	...
	/// MLXChat.app.zip のダウンロード URL（macOS 版が無いリリースでは nil）。
	public let appAssetURL: URL?
	/// MLXChat.ipa のダウンロード URL（iOS 版が無いリリースでは nil）。
	public let ipaAssetURL: URL?
	/// リリースのタイトル（表示用）。
	...
	public init(
		...
		appAssetURL: URL?, ipaAssetURL: URL?, title: String, builtAt: String?)
	{
		self.branch = branch
		self.tag = tag
...
{
	/// リリースに添付される macOS アプリのアセット名（build.yml と一致させる）。
	public static let appAssetName = "MLXChat.app.zip"
	/// リリースに添付される iOS アプリのアセット名（build.yml と一致させる）。
	public static let ipaAssetName = "MLXChat.ipa"

	/// releases API（GET /repos/{owner}/{repo}/releases）の JSON からチャンネル
	/// 一覧を作る。stable / dev-* 以外のタグ、アプリのアセットが 1 つも無い
	/// リリースは黙って除外する（更新先として提示できないため）。結果は stable を
	/// 先頭に、開発版はブランチ名順 — 入力順（API の返却順）に依存しない決定的な並びにする。
	public static func channels(fromReleasesJSON data: Data) throws -> [UpdateChannel]
	{
		...
		for release in releases
		{
			...
			let appURL = assetURL(named: appAssetName, in: release)
			let ipaURL = assetURL(named: ipaAssetName, in: release)
			guard appURL != nil || ipaURL != nil
			else
			{
				continue
			}
	...
	static func assetURL(named name: String, in release: GitHubRelease) -> URL?
	{
