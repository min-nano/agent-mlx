...
- **文字入力のある画面には、キーボードを畳む手段を必ず用意する。** iOS は
  ...
  **一度入力欄に触れたら他の画面へ移れなくなる**（実機で踏んだ）。置き方は
  画面の形で決まる。
  - 下端に固定物が無いフォーム系（設定・ベンチマーク）→ `.dismissibleKeyboard()`
    （`Apps/MLXChatUI/KeyboardDismiss.swift`）
  - 下端に入力欄と送信ボタンを固定している画面（チャット）→ **入力欄の外
    （履歴）を触ったら閉じる。**専用のボタンは置かない。入力欄の並びに置くと
    入力欄が狭くなり、キーボード上のツールバーに置くと送信ボタンと重なる
    （どちらも実機で試して駄目だった）。実装は `ChatView.transcript` の
    `simultaneousGesture`。
...
- **タグと見なすのは応答の冒頭の 1 ブロックだけ**（`ReasoningSplitter.Stage`）。
  本文が 1 文字でも始まったら、以降の `<think>` / `</think>` は**文字列として
  そのまま出す**。復号後の文字列では「タグとして出した」のか「文字として
  書いた」のかを区別できず（エスケープが無い）、間違えたときの損が小さいのは
  こちら — 消す側に倒すと、本文がタグに言及しただけで文が欠けたり、以降の
  本文が丸ごと思考の箱へ消えたりする。壊れたモデルが 2 度目の `</think>` を
  出しても、画面にタグが 1 つ出るだけで済む。
- **本文と思考の前後の空白は `ReasoningSplitter` が落とす。** 推論モデルは
  `</think>` のあとに改行を置いてから答えを書き始めるので、そのまま流すと
  吹き出しの先頭に空行ができる（実機で見えた）。ストリーミングでは
  「これが最後の断片か」が分からないので `trimmingCharacters` は使えず、
  **末尾の空白を保留して次に空白以外が来たら出す**方式にしてある。
  画面側で `trimmingCharacters` を掛け直さないこと（保存済みの本文と食い違う）。
- **思考の開閉を自動でやらない。** 既定は畳む。開閉が変わるのは利用者が
  操作したときだけで、記憶は `ReasoningDisclosure`（Core）が持つ。
  生成の途中で自動的に畳むと数千文字ぶんの高さが一度に消え、末尾へ寄せ続けて
  いるスクロールが**内容より下**に取り残されて画面が空白になる（実機で踏んだ）。
  根拠は [docs/design.md 第 6 節](docs/design.md#開閉は自動でやらないreasoningdisclosure)。
- **開閉の記憶を行（`MessageRow`）の `@State` に置かない。** `LazyVStack` は
  画面外の行の状態を捨てるので、勝手に畳まれて上と同じことが起きる。
- **履歴の追従は末尾の目印（高さ 1pt の `Color.clear`）へ寄せる。**
  発言の id を目標にしない（高さが刻々と変わり、画面外では描画されていない
  ため、寄せた先が内容より下になり得る）。アニメーションも付けない
  — 目標が毎トークン動くので、滑らせると追いつけない。
