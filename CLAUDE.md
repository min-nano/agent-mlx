...
- **タグは常に両方（`<think>` と `</think>`）を探す。** 状態に応じて片方だけを
  探すと、思考の外側に来た `</think>` を見つけられず**タグがそのまま本文に出る**
  （壊れかけの小さいモデルは閉じタグを 2 回出す。実機で見えた）。タグは状態を
  決めるためのもので、本文にも思考にも出さない。保留（割れたタグ）の判定も
  両方について取る — `</th` は `<think>` の接頭辞ではないため。
