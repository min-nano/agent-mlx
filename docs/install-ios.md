...
**Apple ID を Xcode に追加しただけでは、開発用の証明書もプロファイルも作られません。**
最初に実機向けの署名を要求したときに初めて作られます。加えて無料アカウント
（Personal Team）では、**そのデバイスを開発用に登録する**手続きも要ります。この
2 つは Xcode の GUI が面倒を見てくれます。

`scripts/run-ios.sh` は Team ID を次の 3 か所から探しますが、上の理由でどれも
最初は空です。

| 探す場所 | いつ現れるか |
| --- | --- |
| Xcode の設定（`IDEProvisioningTeams`） | Apple ID を追加したとき（入らないこともある） |
| プロビジョニングプロファイル | 初めて実機向けに署名したとき |
| キーチェーンの開発用証明書（OU） | 同上 |

一度 GUI から実機ビルドを通せば、以降はスクリプトが自動で見つけます。
`scripts/run-ios.sh --list` で確認できます。

Team ID が分かっているなら、GUI を通さずに直接指定しても構いません。
