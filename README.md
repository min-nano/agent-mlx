...
します。既定は畳んだ状態で、開くかどうかは操作したときだけ変わります（自動で
開閉すると、生成中に履歴の高さが一度に変わってスクロール位置が飛ぶため）。
思考はモデルへの履歴には渡しません — コンテキストを浪費して品質も落ちるためです。
...
手元に Xcode があるなら方法 A が確実です。**一度 GUI から実機で動かせたら、
以降は `scripts/run-ios.sh` だけでビルド → 転送 → 起動まで通ります。**
...
- 既定は**畳んだ状態**です。考えている間は見出しが「考えています…（N 文字）」に
  変わり、文字数が伸びていくので、止まっていないことはそこで分かります
- 開いたら、生成が終わっても**勝手には畳まれません**
- **自動では開閉しません。** 生成の途中で畳むと数千文字ぶんの高さが一度に消え、
  末尾を追っているスクロールが内容より下に取り残されて画面が空白になるためです
  （[docs/design.md 第 6 節](docs/design.md#開閉は自動でやらないreasoningdisclosure)）
