...
//  という本末転倒になった。そちらは**入力欄の外を触ったら閉じる**形にしてある
//  （`ChatView.transcript` の simultaneousGesture）。畳むボタンを入力欄の並びに
//  置く案も試したが、入力欄が狭くなるので却下した。
//
//  この修飾子が向くのは、下端に固定物が無いフォーム系の画面（設定・ベンチマーク）。
//
